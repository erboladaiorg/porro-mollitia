[![npm version](https://badge.fury.io/js/@erboladaiorg/porro-mollitia.svg)](http://badge.fury.io/js/@erboladaiorg/porro-mollitia)
[![Node.js CI](https://github.com/erboladaiorg/porro-mollitia/actions/workflows/node.js.yml/badge.svg)](https://github.com/erboladaiorg/porro-mollitia/actions/workflows/node.js.yml)
[![Windows Tests](https://img.shields.io/appveyor/ci/tulios/@erboladaiorg/porro-mollitia.svg?label=Windows%20Tests)](https://ci.appveyor.com/project/tulios/@erboladaiorg/porro-mollitia)
# Mappersmith
...
- [Usage](#usage)
  ...
  - [Testing Mappersmith](#testing-@erboladaiorg/porro-mollitia)
  - [Gateways](#gateways)
...
```sh
npm install @erboladaiorg/porro-mollitia --save
```

or

```sh
yarn add @erboladaiorg/porro-mollitia
```
...
```typescript
import forge, { configs } from "@erboladaiorg/porro-mollitia"
import { Fetch } from "@erboladaiorg/porro-mollitia/gateway/fetch"
...
```javascript
const forge = require("@erboladaiorg/porro-mollitia").default;
const { configs } = require("@erboladaiorg/porro-mollitia");
const FetchGateway = require("@erboladaiorg/porro-mollitia/gateway/fetch").default;
```
...
import { configs } from '@erboladaiorg/porro-mollitia'
...
* `request()` - Returns the original [Request](https://github.com/erboladaiorg/porro-mollitia/blob/master/src/request.js)
...
The `prepareRequest` method receives a function which returns a `Promise` resolving the [Request](https://github.com/erboladaiorg/porro-mollitia/blob/master/src/request.js). This function must return a `Promise` resolving the request. The method `enhance` can be used to generate a new request based on the previous one.
...
The `response` method receives a function which returns a `Promise` resolving the [Response](https://github.com/erboladaiorg/porro-mollitia/blob/master/src/response.js). This function must return a `Promise` resolving the Response. The method `enhance` can be used to generate a new response based on the previous one.
...
Why is it not safe? Basically, the setContext function mutates a global state (see [here](https://github.com/erboladaiorg/porro-mollitia/blob/2.34.0/src/@erboladaiorg/porro-mollitia.js#L114)), hence it is the last call to setContext that decides its global value. Which leads to a race condition when handling concurrent requests.
...
"renew" can only be invoked sometimes before it's considered an infinite loop, make sure your middleware can distinguish an error from a "renew". By default, @erboladaiorg/porro-mollitia will allow 2 calls to "renew". This can be configured with `configs.maxMiddlewareStackExecutionAllowed`. It's advised to keep this number low. Example:
...
If an infinite loop is detected, @erboladaiorg/porro-mollitia will throw an error.
...
```javascript
import { BasicAuthMiddleware } from '@erboladaiorg/porro-mollitia/middleware'
...
```javascript
import { CsrfMiddleware } from '@erboladaiorg/porro-mollitia/middleware'
...
```javascript
import { DurationMiddleware } from '@erboladaiorg/porro-mollitia/middleware'
...
```javascript
import { EncodeJsonMiddleware } from '@erboladaiorg/porro-mollitia/middleware'
...
```javascript
import { GlobalErrorHandlerMiddleware, setErrorHandler } from '@erboladaiorg/porro-mollitia/middleware'
...
```javascript
import { LogMiddleware } from '@erboladaiorg/porro-mollitia/middleware'
...
```javascript
import { RetryMiddleware } from '@erboladaiorg/porro-mollitia/middleware'
...
```javascript
import { TimeoutMiddleware } from '@erboladaiorg/porro-mollitia/middleware'
...
This section is only relevant for @erboladaiorg/porro-mollitia versions older than but not including `2.27.0`, when the method `prepareRequest` did not exist. This section describes how to create a middleware using older versions.

Since version `2.27.0` a new method was introduced: `prepareRequest`. This method aims to replace the `request` method in future versions of @erboladaiorg/porro-mollitia, it has a similar signature as the `response` method and it is always async. All previous middleware are backward compatible, the default implementation of `prepareRequest` will call the `request` method if it exists.

The `request` method receives an instance of the [Request](https://github.com/erboladaiorg/porro-mollitia/blob/master/src/request.js) object and it must return a Request. The method `enhance` can be used to generate a new request based on the previous one.
...
## <a name="testing-@erboladaiorg/porro-mollitia"></a> Testing Mappersmith
...
```javascript
import { install, uninstall } from '@erboladaiorg/porro-mollitia/test'
...
```javascript
import { forge } from '@erboladaiorg/porro-mollitia'
import { install, uninstall, mockClient } from '@erboladaiorg/porro-mollitia/test'
...
```javascript
import { m } from '@erboladaiorg/porro-mollitia/test'
...
```javascript
import { forge } from '@erboladaiorg/porro-mollitia'
import { install, uninstall, mockRequest } from '@erboladaiorg/porro-mollitia/test'
...
```javascript
import { install, uninstall, unusedMocks } from '@erboladaiorg/porro-mollitia/test'
...
You can write your own gateway, take a look at [XHR](https://github.com/erboladaiorg/porro-mollitia/blob/master/src/gateway/xhr.js) for an example. To configure, import the `configs` object and assign the gateway option, like:
...
The new configurations will be merged. `configure` also receives the `requestParams` as the first argument. Take a look [here](https://github.com/erboladaiorg/porro-mollitia/blob/master/src/@erboladaiorg/porro-mollitia.js) for more options.
...
Take a look [here](https://github.com/erboladaiorg/porro-mollitia/blob/master/src/@erboladaiorg/porro-mollitia.js) for more options.
...
```javascript
import { FetchGateway } from '@erboladaiorg/porro-mollitia/gateway'
import { configs } from '@erboladaiorg/porro-mollitia'
...
```typescript
import type { Middleware } from '@erboladaiorg/porro-mollitia'
...
```typescript
import { forge } from '@erboladaiorg/porro-mollitia'
import { mockClient } from '@erboladaiorg/porro-mollitia/test'
...
In remote project:
```sh
yarn link ../@erboladaiorg/porro-mollitia/dist
```
...
https://github.com/erboladaiorg/porro-mollitia/graphs/contributors

## License

See [LICENSE](https://github.com/erboladaiorg/porro-mollitia/blob/master/LICENSE) for more details.
